...
Во время всей работы читаем доку по NSOBject, NSArray, NSMutableArray, NSDictionary, NSMutableDictionary, NSString, NSMutabeString, NSSet, NSMutableSet, NSData, NSMutableData
Литералы
Все прорабатываем: c.learncodethehardway.com
...
// многопоточность
// NSThread
// @synchronized и зачем нужен объект, на который синхронизируемся
// NSLock, NSRecursiveLock, NSCondition, NSConditionLock
// prefix.pch
// NSProxy
// Class cluster inheritance
// thread safe proxy
// KVO
// KVC
// associated objects
...
Literature:
1. http://dev.krzysztofprofic.com/tag/nsconditionlock/
2. http://cocoasamurai.blogspot.com/2008/04/osspinlock-lock-showdown-posix-locks-vs.html
3. http://perpendiculo.us/2009/09/synchronized-nslock-pthread-osspinlock-showdown-done-right/
...
Задание 8.
Условие:
Есть иерархия из задания 7.
Есть много мойщиков и много бухгалтеров, бухгалтеров меньше, чем мойщиков. Директор один.
Задание:
...
// блок и владение блоком контекста
// dispatch_queue, dispatch_async, dispatch_sync
// __block переменная

Задание 9.
Условие:
Есть иерархия из задания 8.
Задание:
Надо реализовать все через gcd для асинхронного выполнения.
